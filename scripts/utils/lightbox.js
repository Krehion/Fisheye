function lightbox(galleryData) {
	...
	function updateFocusableElements() {
		...
		const focusableContent = lightboxWrapper.querySelectorAll(
	...
	function attachEventListeners() {
		...
		document.addEventListener("keydown", function (e) {
			...
			if (
				isLeftArrowPressed &&
				lightboxWrapper.getAttribute("aria-hidden") === "false"
			) {
				showPrevMedia();
			}
		});
	...
	function displayLightbox(mediaData) {
		...
		const existingContent = lightboxContainer.querySelector(
			".lightbox-content-wrapper"
		);

		if (existingContent) {
			// remove previously displayed content before generating a new one
